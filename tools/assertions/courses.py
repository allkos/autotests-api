...
def assert_create_course_response(
    ...
    :param create_course_request: Данные, отправленные в запросе.
    :param create_course_response: Ответ API после создания курса.
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    course = create_course_response.course

    assert_equal(course.title, create_course_request.title, "title")
    assert_equal(course.max_score, create_course_request.max_score, "max_score")
    assert_equal(course.min_score, create_course_request.min_score, "min_score")
    assert_equal(course.description, create_course_request.description, "description")
    assert_equal(course.estimated_time, create_course_request.estimated_time, "estimated_time")

    assert_equal(course.preview_file.id, create_course_request.preview_file_id, "preview_file_id")
    assert_equal(course.created_by_user.id, create_course_request.created_by_user_id, "created_by_user_id")